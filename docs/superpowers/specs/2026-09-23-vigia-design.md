...
Vigia ("lookout") is a free, open-source, **self-hosted chat watchman for Twitch
...
- Vigia loads the matching pack automatically when the stream category changes. The
...
- **URL:** `/overlay?key=<token>`. The key is a read-only token generated by Vigia, which
...
- **Server:**
  - Mods sign in with Twitch. Vigia checks that the user is the broadcaster or on the
...
- **Demo:** the demo stays on GitHub Pages and gets a "Download Vigia" link.
